...
def process(mol, id_dict, db):
    ...
    def qn_format(qn_series, extra):
        fmt = u'{:d}({:d}, {:d}) - {:d}({:d}, {:d}), F = {:d} - {:d}, A'
        order = [0, 1, 2, 5, 6, 7, 4, 9]
        return fmt.format(*[int(qn_series[j]) for j in order])

    cursor = db.cursor()
    cmd = "SELECT * FROM species WHERE s_name_noparens LIKE 'CH2CHCN%%'"
    cursor.execute(cmd)
    res = cursor.fetchall()
...
def main():
    ...
    JPL_ID_NUM = 53001
    molecule = JPLMolecule([time.strftime('%Y/%m/%d', time.localtime()), JPL_ID_NUM, 'C2H3CN'])
    ...
    vib_states = {'g.s.': [0], 'v11=1': [1], 'v15=1':[2]}
