...
def process(mol, id_dict, db):
    ...
    cursor = db.cursor()
    cmd = "SELECT * FROM species WHERE s_name_noparens LIKE 'cis-CH2OHCHO%%'"
    cursor.execute(cmd)
    res = cursor.fetchall()
...
def main():
    ...
    JPL_ID_NUM = 60006
    molecule = JPLMolecule([time.strftime('%Y/%m/%d', time.localtime()), JPL_ID_NUM, 'Glycolaldehyde'])
    ...
    vib_states = {'v = 0': [0], 'v = 1': [1], 'v = 2':[2], 'v = 3':[3]}
