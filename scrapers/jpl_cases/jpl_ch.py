...
def process(mol, id_dict, db, qn_format, substring):
    ...
    mask = (mol.cat['qn_up_3']-mol.cat['qn_dwn_3']).isin(vib_states_ids)
...
def main():
    ...
    JPL_ID_NUM = 13002
    molecule = JPLMolecule([time.strftime('%Y/%m/%d', time.localtime()), JPL_ID_NUM, 'CH'])
