...
def process(mol, id_dict, db):
    ...
    def qn_format(qn_series, extra):
        if int(qn_series[2]) == 1 and int(qn_series[6]) == -1:
            ...
            if int(qn_series[1]) > 0:
                ...
                fmt = u'{:d}({:d}) - {:d}({:d}), E2'.encode('utf-8')
        else:
            fmt = u'{:d}({:d}) - {:d}({:d})'.encode('utf-8')
        order = [0, 1, 4, 5]

        return fmt.format(*[int(qn_series[j]) for j in order])

    cursor = db.cursor()
    cmd = "SELECT * FROM species WHERE s_name_noparens LIKE 'CH3OH%%'"
    cursor.execute(cmd)
    res = cursor.fetchall()
...
def main():
    ...
    JPL_ID_NUM = 32003
    molecule = JPLMolecule([time.strftime('%Y/%m/%d', time.localtime()), JPL_ID_NUM, 'CH3OH'])
    ...
    vib_states = {'g.s.': [0], 'vt=1': [1], 'vt=2':[2]}
