...
def process(mol, id_dict, db):
    ...
    def qn_format(qn_series, extra):
        ...
        if vib_id in (0, 1, 3, 4):
            fmt = u'J = {:d} - {:d}'.encode('utf-8')
            order = [0, 3]
        else:
            if int(qn_series[0])-int(qn_series[3]) != 0:
                if int(qn_series[1]) < 0 and int(qn_series[4]) > 0 :
                    fmt = u'J = {:d} - {:d}, l = e'.encode('utf-8')
                elif int(qn_series[1]) > 0 and int(qn_series[4]) < 0:
                    fmt = u'J = {:d} - {:d}, l = f'.encode('utf-8')
                order = [0, 3, 1]

            else:
                if int(qn_series[1]) < 0 and int(qn_series[4]) > 0 :
                    fmt = u'J = {:d}, l = e - f'.encode('utf-8')
                elif int(qn_series[1]) > 0 and int(qn_series[4]) < 0:
                    fmt = u'J = {:d}, l = f - e'.encode('utf-8')
                order = [0, ]
    ...
    cursor = db.cursor()
    cmd = "SELECT * FROM species WHERE s_name_noparens LIKE 'N2O%%'"
    cursor.execute(cmd)
    res = cursor.fetchall()
    ...
    mask = (mol.cat['qn_up_2'].isin(vib_states_ids)) & (mol.cat['qn_dwn_2'].isin(vib_states_ids)) & (mol.cat['qn_up_2'] == mol.cat['qn_dwn_2'])
...
def main():
    ...
    JPL_ID_NUM = 44004
    molecule = JPLMolecule([time.strftime('%Y/%m/%d', time.localtime()), JPL_ID_NUM, 'N2O'])
    ...
    vib_states = {'g.s.': [0], 'v1': [1], 'v2':[2], 'v3': [3], '2v2':[4,5]}
