...
def process(mol, id_dict, db):
    ...
    cursor = db.cursor()
    cmd = "SELECT * FROM species WHERE s_name_noparens LIKE 'CH3CHO%%'"
    cursor.execute(cmd)
    res = cursor.fetchall()
...
def main():
    ...
    JPL_ID_NUM = 44003
    molecule = JPLMolecule([time.strftime('%Y/%m/%d', time.localtime()), JPL_ID_NUM, 'CH3CHO'])
    ...
    vib_states = {'g.s.': [0, 1, 2], 'vt=1': [3, 4, 5], 'vt=2':[6, 7, 8]}
