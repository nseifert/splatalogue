...
class SLAIMMolecule:
    ...
    def get_metadata(self):
        ...
        gui_order = ['Name', 'Contributor', 'Q(300.0)=', 'Q(225.0)=', 'Q(150.0)=', 'Q(75.00)=', 'Q(37.50)=', 'Q(18.75)=',
                     'Q(9.375)=', 'A', 'B', 'C', '$\\mu_a$ =', '$\\mu_b$ =', '$\\mu_c$ =', 'Ref1', 'Ref20']
...
def process(mol, id_dict, db, qn_format, substring):
    key = id_dict[0]
    ...
    if species_id == "X":  # New entry
        ...
        metadata_to_push['Name'] = "%s %s"%(mol.name, key)
    ...
    mask = (mol.cat['qn_up_3']).isin(vib_states_ids) & (mol.cat['qn_dwn_3']).isin(vib_states_ids)
...
def main():
    ...
    spec_data = {'Name': 'O<sub>2</sub><sup>18</sup>O-asym', 'Mass': '50'}

    molecule = SLAIMMolecule(listing_entry=[spec_data['Name'], spec_data['Mass']], cat_file=open(eg.fileopenbox(msg='Select CAT file'),'r'))
    ...
    vib_states = {'v<sub>2</sub> = 0': [0], 'v<sub>2</sub> = 1': [1]}
    ...
    for key in vib_states:
        process(molecule, (key, vib_states), db, qn_format, NAME_SUBSTRING)
